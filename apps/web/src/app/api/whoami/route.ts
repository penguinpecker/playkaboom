 ...
 *
 * Anonymous: returns the env-binding view (which Privy app id is configured,
 * whether the secret env var is set; secrets are NEVER returned, only "set/unset").
 * Authed   : returns userId + linked Solana wallets.
 */
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (err instanceof Error && /Missing auth token|Invalid auth token|Privy lookup failed|User not found/.test(err.message)) {
      return NextResponse.json(
        { authenticated: false, reason: err.message, env },
        { status: 200 },
      );
    }
